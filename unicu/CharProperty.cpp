...
bool CharProperty::open(const std::string& filename) {
  if (!acc::readFile(filename.c_str(), data_)) {
...
size_t CharProperty::id(acc::StringPiece key) const {
...
CharInfo CharProperty::getCharInfo(acc::StringPiece key) const {
  return CharInfo(id(key));
}

char16_t readHex(acc::StringPiece s) {
...
struct CharRange {
  ...
  std::vector<acc::StringPiece> c;  // code point categories
};

CharRange parseCharRangeFromLine(acc::StringPiece line) {
  CharRange range;
  std::vector<acc::StringPiece> cols, v;
...
CharInfo encode(const std::vector<acc::StringPiece>& c,
                const std::map<acc::StringPiece, CharInfo>& m) {
  CharInfo base;
  if (c.empty()) {
...
bool CharProperty::compile(const std::string& cfile,
  ...
  std::string conf;
  if (!acc::readFile(cfile.c_str(), conf)) {
    ACCLOG(ERROR) << "read conf file " << cfile << " failed";
    return false;
  }
  std::vector<CharRange> ranges;
  std::map<acc::StringPiece, CharInfo> cmap;
  std::vector<acc::StringPiece> carray;
  size_t i = 0;
  // parse lines
  std::vector<acc::StringPiece> lines;
  split('\n', conf, lines);
  for (auto& line : lines) {
  ...
  if (!acc::writeFile(out, ofile.c_str())) {
